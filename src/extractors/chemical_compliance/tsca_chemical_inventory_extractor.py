"""Extractor for TSCA Chemical Inventory database"""
...
class TSCAChemicalInventoryExtractor(BaseExtractor):
    """Extract TSCA Chemical Inventory data"""
    ...
    def download(self) -> Optional[Path]:
        """Download TSCA Chemical Inventory data"""
        print(f"    Downloading from {self.url}...")
        
        try:
            # TSCA inventory is available via EPA's website
            # Try to find download links or use EPA API if available
            ...
            # EPA often provides CSV/Excel downloads
            download_urls = [
                "https://www.epa.gov/sites/default/files/2020-01/documents/tsca_inventory_2020.csv",
                "https://www.epa.gov/tsca-inventory/tsca-inventory-data",
            ]
            
            # Try to get the main page and find download links
                    ...
                    if any(ext in href.lower() for ext in ['.csv', '.xlsx', '.xls', 'download']):
                        ...
                        try:
                            ...
                            if file_response.status_code == 200:
                                ...
                                raw_file = self.raw_dir / f"tsca_inventory_{datetime.now().strftime('%Y%m%d')}{ext}"
            ...
            # If no download found, save the HTML for manual processing
            raw_file = self.raw_dir / f"tsca_inventory_{datetime.now().strftime('%Y%m%d')}.html"
            raw_file.write_bytes(response.content)
            print(f"    ⚠️  No direct download found, saved HTML page")
            print(f"    Note: Manual download may be required from {self.url}")
            return raw_file
    ...
    def parse(self, raw_file: Path) -> List[Dict[str, Any]]:
        """Parse TSCA Chemical Inventory data with enhanced HTML parsing"""
    ...
    def to_markdown(self, data: List[Dict[str, Any]]) -> str:
        ...
        md += "---\n\n"
        md += "## TSCA Chemical Inventory\n\n"
        md += "The Toxic Substances Control Act (TSCA) Chemical Inventory contains "
        md += "all chemicals manufactured, processed, or imported in the United States.\n\n"
        md += "## Chemical Entries\n\n"
        
        for entry in data[:1000]:  # Limit to first 1000 for readability
            name = entry.get('chemical_name', 'Unknown')
            cas = entry.get('cas_number', 'N/A')
            
            md += f"### {name}\n\n"
            md += f"- **CAS Number:** {cas}\n"
            
            if entry.get('molecular_formula'):
                md += f"- **Molecular Formula:** {entry['molecular_formula']}\n"
            if entry.get('molecular_weight'):
                md += f"- **Molecular Weight:** {entry['molecular_weight']}\n"
            if entry.get('inventory_status'):
                md += f"- **Inventory Status:** {entry['inventory_status']}\n"
