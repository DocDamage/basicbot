"""Uspto Patent Database Extractor"""
...
class USPTOPatentDatabaseExtractor(BaseExtractor):
    """Extract USPTO Patent Database data"""
    ...
    def download(self) -> Optional[Path]:
        """Download USPTO Patent Database data using API"""
        ...
        try:
            ...
            # Try bulk data downloads first
            api_urls = [
                "https://bulkdata.uspto.gov/data/patent/grant/redbook/fulltext/",  # Full text patents
                "https://developer.uspto.gov/",  # API documentation
            ]
            
            # Try to get sample data or API info
                    ...
                    if response.status_code == 200:
                        if 'json' in response.headers.get('content-type', '').lower():
                            ...
                            for link in soup.find_all('a', href=True):
                                href = link['href']
                                if any(ext in href.lower() for ext in ['.xml', '.json', '.zip', 'download', 'bulk']):
                                    if href.startswith('/'):
                                    ...
                                    try:
                                        ...
                                        if file_response.status_code == 200:
                                            ext = '.xml' if '.xml' in href.lower() else ('.json' if '.json' in href.lower() else ('.zip' if '.zip' in href.lower() else '.html'))
            ...
            if response.status_code == 200:
                ...
                print(f"    Note: USPTO API available at https://developer.uspto.gov/ (requires registration)")
                return raw_file
    ...
    def parse(self, raw_file: Path) -> List[Dict[str, Any]]:
        """Parse USPTO Patent Database data"""
